...
class VPNManager:
    ...
    async def _connect_wireguard(self, interface: Optional[str] = None) -> bool:
        ...
        if result.returncode == 0:
            ...
            logger.error("WireGuard connection failed: %s", result.stderr)
            return False
    ...
    async def disconnect_vpn(self) -> bool:
        ...
        try:
            if self.current_vpn == "openvpn":
                await asyncio.to_thread(subprocess.run, ["sudo", "killall", "openvpn"])
            elif self.current_vpn == "wireguard":
                await asyncio.to_thread(subprocess.run, ["sudo", "wg-quick", "down", "wg0"])
            elif self.current_vpn == "protonvpn":
                await asyncio.to_thread(subprocess.run, ["protonvpn", "disconnect"])
            elif self.current_vpn == "nordvpn":
                await asyncio.to_thread(subprocess.run, ["nordvpn", "disconnect"])
...
class StealthMode:
    ...
    async def enable_stealth(self, vpn_server: Optional[str] = None) -> bool:
        ...
        if self.vpn.available_vpns:
            ...
            logger.warning("No VPN available, continuing without VPN")
