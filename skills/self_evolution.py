...
class SelfEvolutionSkill(BaseSkill):
    ...
    inputs = {
        "objective": "The feature or improvement to implement.",
        "files": "(Optional) Specific files to target."
    }
    ...
    async def execute(self, goal: Dict[str, Any], context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        objective = params.get("objective") or goal.get("objective")
        ...
        if action == "apply":
            return await self._apply_evolution(objective, params.get("files"), brain, context)
